...
const passedDemandFeedback = sequential(passedPrice, (item) => (item.metrics?.recentVolume == null || Number(item.metrics.recentVolume) > 0) && (item.metrics?.feedbackPct == null || Number(item.metrics.feedbackPct) >= 95));
...
const passedProfitability = sequential(passedNicheIntent, (item) => item.metrics?.commissionAmountGbp == null && item.metrics?.commissionRatePct == null ? true : Number(item.metrics?.commissionAmountGbp) >= 0.5 || (Number(item.metrics?.priceGbp) * Number(item.metrics?.commissionRatePct) / 100) >= 0.5);
