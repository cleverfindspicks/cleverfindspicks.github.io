...
3. Qualification calculates the 100-point score and applies the hard filters without requiring a creative. Only a provisional winner proceeds to official Affiliate link generation from its canonical product URL and an end-to-end redirect check. The final publication gate requires an exact destination Product ID match, a reviewed vertical pin, complete landing-page content and RSS data.

`pnpm intelligence:affiliate-audit` follows every published promotion link, records the redirect chain and final destination, repairs a broken link only when the same exact Product ID is known, and otherwise disables the CTA without deleting the Clever Finds page. `pnpm intelligence:automation` runs this audit before each scheduled candidate search.
