...
export async function exportHub(db){
  ...
  const safe=rows.map(r=>{const c=JSON.parse(r.evidence_json||'{}').candidate;return {instagramPublicationId:r.instagram_publication_id,trackingId:r.internal_instagram_tracking_id,productId:r.product_id,productSlug:r.product_slug,creativeId:r.creative_id,publishedAt:r.published_at,mediaId:r.media_id,permalink:r.permalink,hook:r.hook,layoutFamily:r.layout_family,mediaType:r.media_type||'IMAGE',videoSourceType:r.video_source_type||null,platform:r.platform||'instagram',category:r.category,cluster:r.cluster,hashtags:JSON.parse(r.hashtags_json||'[]'),keywords:JSON.parse(r.keywords_json||'[]'),currencyVerified:currencyGate(c||{}),productApproved:c?.decision==='keep'};});
