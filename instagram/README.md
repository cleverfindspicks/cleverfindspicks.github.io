...
8. `pnpm instagram:trial` is reserved for an explicitly requested manual test or preview. Scheduled Production is `FULL_AUTO`: it generates and validates the real-product creative automatically, retries creation twice, and publishes without manual creative approval only when every Product, Currency, Affiliate, Image, Variant and Instagram Suitability gate passes. Scheduler registration remains separate.
