...
export async function refreshProductEvidence(choices,{fetcher=fetch}={}){
  ...
  const fresh=choices.flatMap(({product,candidate})=>{
    ...
    const updated={...candidate,...v,currencyVerification:v,metrics:{...commissionMetrics(item,v),feedbackPct:Number.parseFloat(item.evaluate_rate)||null,recentVolume:Number(item.lastest_volume)||0},productUrl:item.product_detail_url,image:item.product_main_image_url||candidate.image,officialProductVideoUrl:item.product_video_url||candidate.officialProductVideoUrl||null,productVideoProductId:item.product_video_url?String(item.product_id):(candidate.productVideoProductId||null),productVideoSourceBasis:item.product_video_url?'ALIEXPRESS_OFFICIAL_PRODUCT_DETAIL':(candidate.productVideoSourceBasis||null),affiliateUrl:product.affiliateUrl,canonicalProductUrl:product.canonicalProductUrl,affiliateDestinationVerified:product.affiliateDestinationVerified,detailVerification:{productIdMatched:String(item.product_id)===String(product.productId),priceMatched:v.currency_verified,checkedAt:v.currency_verification_timestamp},expectedPriceBandGbp:null};
